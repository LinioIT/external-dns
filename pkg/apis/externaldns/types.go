...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("source", "The resource types that are queried for endpoints; specify multiple times for multiple sources (required, options: service, ingress, fake, connector, istio-gateway, istio-virtual-service, cloudfoundry, crd, empty").Required().PlaceHolder("source").EnumsVar(&cfg.Sources, "service", "ingress", "istio-gateway", "istio-virtual-service", "cloudfoundry", "fake", "connector", "crd", "empty")
