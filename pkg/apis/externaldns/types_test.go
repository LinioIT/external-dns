...
var (
	...
	// minimal config with istio gateway source and multiple ingressgateway load balancer services
	multipleIstioIngressGatewaysVirtualServiceConfig = &Config{
		Master:                      "",
		KubeConfig:                  "",
		RequestTimeout:              time.Second * 30,
		IstioIngressGatewayServices: []string{"istio-system/istio-ingressgateway", "istio-other/istio-otheringressgateway"},
		Sources:                     []string{"istio-virtual-service"},
		Namespace:                   "",
		FQDNTemplate:                "",
		Compatibility:               "",
		Provider:                    "google",
...
func TestParseFlags(t *testing.T) {
	for _, ti := range []struct {
		...
		{
			title: "istio virtual service config with 2 ingressgateways",
			args: []string{
				"--provider=google",
			...
			envVars:  map[string]string{},
			expected: multipleIstioIngressGatewaysVirtualServiceConfig,
		},
		{
